...
export async function middleware(request: NextRequest) {
    ...
    if (!session) {
        console.log('Admin middleware: No session cookie found, redirecting to auth');
  ...
  try {
    ...
    if (!adminAuth || typeof adminAuth.verifySessionCookie !== 'function') {
      console.log('Admin middleware: Firebase Admin SDK not initialized, redirecting to auth');
      return NextResponse.redirect('https://auth.arya.services/login');
    }

    // Verify the Firebase session cookie
    const decoded = await adminAuth.verifySessionCookie(session, true);
    console.log('Admin middleware: Session verified for user:', decoded.uid);
    ...
    console.log('Admin middleware: Session verification failed:', error);
    return NextResponse.redirect('https://auth.arya.services/login');
  }
}
